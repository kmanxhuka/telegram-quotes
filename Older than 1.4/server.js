...
app.get('/quotes', (req, res) => {
  ...
  const bodyText = lines.length > 1 ? lines.slice(0, -1).join('\n') : randomQuote.text;

  const lastLineOffset = randomQuote.text.length - rawTitle.length - 1; // -1 for \n
  const filteredEntities = (randomQuote.entities || []).filter(e => e.offset + e.length <= lastLineOffset);

  const html = formatTextWithEntities(bodyText, filteredEntities);
